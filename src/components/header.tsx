...
import { useEffect, useState, RefAttributes, ForwardRefExoticComponent, Fragment } from 'react';
...
// --- UPDATED: Navigation Items Configuration Restored ---
const navItems: NavItem[] = [
  ...
  { name: 'Professionals', href: '/professionals', icon: Users },
  {
    ...
    icon: Building,
    dropdown: [
      { name: 'All Organizations', href: '/organizations', description: 'Browse all institutions' },
      { name: 'Companies', href: '/organizations?type=Company', description: 'Leading private sector firms' },
  ...
  },
  {
    ...
    dropdown: [
      ...
      { name: 'Heritage', href: '/heritage', description: 'Cultural and historical initiatives' },
    ],
  },
                ...
                {!loading && (
                    session ? (
                      ...
                      <>
                        ...
                        <Link href="/register" className="px-4 py-2 text-sm font-semibold text-white bg-amber-500 rounded-lg hover:bg-amber-600 transition-all flex items-center gap-2"><UserPlus size={16}/> Sign Up</Link>
                      </>
                    )
                )}
              </div>
          ...
          <>
            ...
            <motion.div variants={mobileMenuVariants} initial="hidden" animate="visible" exit="exit" className="fixed top-0 right-0 bottom-0 w-full max-w-sm bg-white shadow-xl z-50 flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto">
                <motion.nav className="flex flex-col space-y-1 p-6">
                  {navItems.map((item) => (
                    <div key={item.name}>
                      {item.dropdown ? (
                  ...
                  {!loading && (
                      session ? (
                        ...
                        <>
                          ...
                          <Link href="/register" onClick={() => setIsMobileMenuOpen(false)} className="w-full text-center px-4 py-3 font-semibold text-white bg-amber-500 rounded-lg flex items-center justify-center gap-2"><UserPlus size={16}/> Sign Up</Link>
                        </>
                      )
                  )}
                </div>

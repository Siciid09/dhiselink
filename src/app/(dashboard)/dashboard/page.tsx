...
import { 
    PlusCircle, Briefcase, Users, Eye, Edit, Building, BookOpen, Heart, Shield, Landmark, Handshake, Lightbulb 
} from 'lucide-react';

// This is the "master switch" that controls what each role can do and see.
export const roleConfig: Record<string, any> = {
    individual: {
        listTitle: "My Recent Ideas",
        actions: [
            { name: "Manage My Ideas", href: "/dashboard/manage", icon: Lightbulb },
            { name: "Profile Settings", href: "/dashboard/settings", icon: Users }
        ],
        creatableContentTypes: [
            { type: 'idea', title: 'Idea', description: 'Submit an idea for a project or venture.', icon: 'Lightbulb' }
        ]
    },
    company: {
    ...
    ngo_gov: {
        ...
        creatableContentTypes: [
            ...
            { type: 'project', title: 'Project / Event / Tender', description: 'Detail an initiative or public notice.', icon: 'Shield' }
        ]
    },
    other: {
...
// ... (The rest of your dashboard page component remains unchanged) ...
// (DashboardSkeleton, DashboardContent, etc.)
...
const summaryTypeConfig: Record<string, { className: string }> = {
    ...
    idea: { className: 'bg-amber-100 text-amber-800'},
...
async function DashboardContent() {
    ...
    // Use the roleConfig to get role-specific settings
    const config = roleConfig[profile.role] || roleConfig.company; // Default to company if role is unknown
    
    // (The data fetching logic for the summary list can also be updated later to be role-specific)
    ...
    const statCards = [ { title: "Total Postings", value: 0, icon: Briefcase }, { title: "Total Applicants", value: 0, icon: Users }, { title: "Profile Views", value: 0, icon: Eye } ];
                        ...
                        <div className="space-y-4">
                            {mainListItems.length > 0 ? (
                                mainListItems.map(item => (
                                    <div key={item.id} className="p-4 rounded-lg bg-slate-50 border">

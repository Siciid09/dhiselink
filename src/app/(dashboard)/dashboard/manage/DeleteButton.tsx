...
export default function DeleteButton({ item }: { item: { id: string, type: string } }) {
    ...
    // Pre-bind the arguments to the server action
    const deleteItemWithId = () => deleteItem(item.id, item.type);
    ...
    return (
        <>
            <button onClick={() => setIsOpen(true)} title="Delete" className="p-2 text-slate-500 hover:bg-red-100 hover:text-red-600 rounded-md transition-colors">
                <Trash2 size={16} />
            </button>
            <AnimatePresence>
                {isOpen && (
                    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-xl p-8 shadow-2xl max-w-sm w-full">
                            <div className="flex items-center gap-3"><div className="flex-shrink-0 p-3 bg-red-100 rounded-full"><AlertTriangle className="text-red-600 h-6 w-6" /></div><h2 className="text-xl font-bold text-slate-800">Confirm Deletion</h2></div>

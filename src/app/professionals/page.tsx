...
import { Search, ListFilter, Loader2, ArrowRight, User } from 'lucide-react';
...
const ProfessionalCard: FC<{ profile: ProfessionalProfile }> = ({ profile }) => {
  const shortDescription = profile.bio?.split(' ').slice(0, 12).join(' ') + (profile.bio && profile.bio.split(' ').length > 12 ? '...' : '');

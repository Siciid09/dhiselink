...
export default async function ProfessionalDetailPage({ params }: { params: { slug: string } }) {
    ...
    const socialLinks = [ { href: profile.website_url, icon: Globe }, { href: profile.linkedin_url, icon: Linkedin }, { href: profile.github_url, icon: Github }].filter(link => link.href);
            ...
            <div className="container mx-auto max-w-5xl py-16 sm:py-24 px-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 space-y-8">
                        <ProfileSection title="About Me" icon={User} hasContent={!!profile.bio}><p className="text-slate-600 leading-relaxed whitespace-pre-line">{profile.bio}</p></ProfileSection>
                        ...
                        <ProfileSection title="Work Experience" icon={Briefcase} hasContent={hasWorkExperience}>
                            <div className="space-y-6 border-l-2 border-slate-200 ml-2">
                                {/* --- THIS IS THE FIX --- */}
                                {/* The '?.' ensures this code doesn't run if work_experience is null */}

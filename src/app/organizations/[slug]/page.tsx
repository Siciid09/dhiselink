...
export default async function OrganizationDetailPage({ params }: { params: { slug: string } }) {
    ...
    const profilePromise = supabase.from('profiles').select('*').eq('slug', slug).single();
    ...
    const id = profile.id; // We still need the ID to fetch related items

    const jobsPromise = supabase.from('jobs').select('*').eq('organization_id', id).order('created_at', { ascending: false });
    const programsPromise = supabase.from('programs').select('*').eq('organization_id', id).order('created_at', { ascending: false });
    const initiativesPromise = supabase.from('initiatives').select('*').eq('organization_id', id).order('created_at', { ascending: false });
    const servicesPromise = supabase.from('services').select('*').eq('organization_id', id).order('created_at', { ascending: false });
    const galleriesPromise = supabase.from('galleries').select('*').eq('organization_id', id).order('created_at', { ascending: false });
    ...
    const displayType = profile.organization_type;
    const displaySubtype = profile.organization_subtype;

    // ... The rest of the page component's JSX remains exactly the same ...
    return (
        <div className="bg-slate-50 min-h-screen">
            <div className="container mx-auto max-w-5xl py-16 px-4">
                ...
                <header className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
                    ...
                    <div className="p-6 sm:p-8 relative"><img src={profile.logo_url || `https://api.dicebear.com/8.x/initials/svg?seed=${displayName}`} alt="Logo" className="w-24 h-24 sm:w-32 sm:h-32 rounded-2xl border-4 border-white shadow-lg object-contain bg-white absolute -top-12 sm:-top-16"/>
                        <div className="pt-16 sm:pt-20">
                            ...
                            {profile.tags && profile.tags.length > 0 && (<div className="flex flex-wrap items-center gap-2 mt-4">{profile.tags.map((tag: string) => (<span key={tag} className="text-xs font-semibold bg-sky-100 text-sky-800 px-3 py-1 rounded-full">{tag}</span>))}</div>)}
                ...
                <main className="mt-8">
                    <OrganizationTabs profile={profile} jobs={jobs || []} programs={programs || []} initiatives={initiatives || []} services={services || []} galleries={galleries || []}/>
                </main>
            </div>
        </div>
    );

            ...
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-3xl pt-24">
                ...
                <form action={updateUserRole}>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                         {roles.map((role) => (
                            <label key={role.name} className={`p-6 border-2 rounded-xl cursor-pointer transition-all ${selectedRole === role.name ? 'border-blue-600 bg-blue-50 ring-4 ring-blue-200' : 'border-gray-200 bg-white hover:border-blue-400'} ${role.name === 'other' ? 'lg:col-start-2' : ''}`}>
                                <input type="radio" name="role" value={role.name} className="hidden" onChange={() => setSelectedRole(role.name)} />
                                <div className="flex items-center gap-4">
                                    <div className={`p-3 rounded-lg ${selectedRole === role.name ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}>{role.icon}</div>
                                    <div>
                    ...
                    <div className="mt-10 pt-4 text-center">
                        <motion.button type="submit" disabled={!selectedRole} className="w-full max-w-xs px-8 py-4 bg-blue-600 text-white font-bold rounded-lg shadow-lg disabled:bg-gray-400" whileHover={{ scale: 1.05 }}>
                            Continue
                        </motion.button>

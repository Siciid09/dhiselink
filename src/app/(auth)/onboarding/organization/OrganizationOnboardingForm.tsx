...
function SubmitButton() {
    ...
    return <button type="submit" disabled={pending} className="px-6 py-3 bg-green-600 text-white font-bold rounded-lg disabled:bg-gray-400">{pending ? "Saving..." : "Complete Setup"}</button>;
}
            ...
            <div className="w-full max-w-3xl bg-white rounded-2xl shadow-xl border p-8">
                <div className="mb-8">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                        <motion.div className="bg-blue-500 h-2 rounded-full" animate={{ width: `${(currentStep / 4) * 100}%` }} />
                    </div>
                </div>
...
const Tab1 = ({ onNext, initialData }: any) => {
    ...
    return (
        <div>
            <h2 className="text-3xl font-bold mb-6">Organization Identity</h2>
            <div className="space-y-4">
                <div><label className="font-medium">Official Organization Name</label><input name="organization_name" value={data.organization_name || ''} onChange={handleChange} className="w-full mt-1 p-3 border rounded-lg" required /></div>
                <div><label className="font-medium">Headquarters Location</label><input name="location" value={data.location || ''} onChange={handleChange} className="w-full mt-1 p-3 border rounded-lg" required /></div>
                <div><label className="font-medium">Year Founded</label><input name="year_founded" type="number" value={data.year_founded || ''} onChange={handleChange} className="w-full mt-1 p-3 border rounded-lg" /></div>
            </div>
...
const Tab2 = ({ onNext, onPrev, initialData, orgSubtypes }: any) => {
    ...
    return (
        <div>
            <h2 className="text-3xl font-bold mb-6">Profile & Mission</h2>
            <div className="space-y-4">
                <div><label className="font-medium">Organization Subtype</label>
                ...
                <div><label className="font-medium">About Us / Mission (Bio)</label><textarea name="bio" rows={4} value={data.bio || ''} onChange={handleChange} className="w-full mt-1 p-3 border rounded-lg" required></textarea></div>
                <div><label className="font-medium">Number of Employees</label><input name="employee_count" type="number" value={data.employee_count || ''} onChange={handleChange} className="w-full mt-1 p-3 border rounded-lg" /></div>
            </div>
...
const Tab3 = ({ onNext, onPrev, initialData }: any) => {
    ...
    return (
        <div>
            ...
            <div className="space-y-4">
                <div><label className="font-medium">Official Website URL</label><input name="website_url" type="url" value={data.website_url || ''} onChange={handleChange} className="w-full mt-1 p-3 border rounded-lg" required /></div>
                <div><label className="font-medium">Public Contact Email</label><input name="email" type="email" value={data.email || ''} onChange={handleChange} className="w-full mt-1 p-3 border rounded-lg" /></div>
            </div>
            <div className="mt-8 flex justify-between">
                <button type="button" onClick={onPrev} className="px-6 py-3 bg-gray-200 font-bold rounded-lg">Previous</button>
                <button type="button" onClick={() => onNext(data)} className="px-6 py-3 bg-blue-600 text-white font-bold rounded-lg">Next</button>
